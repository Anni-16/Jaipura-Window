...
if (isset($_GET['ecat_id'])) {
    $url = $_GET['ecat_id'];

    // Fetch the subcategory details based on the URL parameter
    $statement = $pdo->prepare("SELECT * FROM tbl_end_category WHERE ecat_id = ? AND `show` = 1");
    $statement->execute([$url]);
    $endCategory = $statement->fetch(PDO::FETCH_ASSOC);
    ...
    $ecat_id = $endCategory['ecat_id'];
    $ecat_name = $endCategory['ecat_name'];
    $mcat_id = $endCategory['mcat_id'];

    // Fetch the related top and mid-category details
    $statement = $pdo->prepare("
        SELECT t1.*, t2.mcat_name, t3.tcat_name
    ...
    $categoryDetails = $statement->fetch(PDO::FETCH_ASSOC);

    if ($categoryDetails) {
        $mcat_name = $categoryDetails['mcat_name'];
        $tcat_name = $categoryDetails['tcat_name'];
    }
...
<head>
    ...
    <title>Welcome To Jaipur Window - Wear Categories Filtaration Products | Jaipur Window</title>
        ...
        <div class="breadcrumb-main">
            <div class="container">
                <div class="breadcrumb-container">
                    <h2 class="page-title"><?= $ecat_name; ?></h2>
                    <ul class="breadcrumb">
            ...
            <div class="row">
                ...
                <div id="content" class="col-sm-9  all-blog my-baby-pink" style="padding: 20px 40px;">
                    <div class="category-refine">
                        <h3><strong>Refine Search By Wear End Categories</strong></h3>
                        <div class="row">
                            <div class="col-sm-12">
                                <ul>
                                    <?php
                                    $i = 0;
                                    $statement = $pdo->prepare("SELECT * FROM tbl_end_category WHERE mcat_id =? AND `show` = 1 ORDER BY ecat_name ASC");
                                    ...
                                    ?>
                                        <li>
                                            <a href="wear-end-category.php?ecat_id=<?= $row['ecat_id']; ?>"><?= $row['ecat_name']; ?></a>
                                        </li>
                                    <?php } ?>
                                </ul>
                            </div>
                    ...
                    <div id="items-container">
                                        ...
                                        <div class="food-cart-box" data-product-id="1">
                                            <div class="food-box">
                                                <div class="product-thumb">
                                                    <div class="image">
                                                        <a href="wear-details.php?p_id=<?= $product['p_id']; ?>" class="thumb-image">
                                                            <?php if (!empty($product['p_featured_photo']) && file_exists('./admin/uploads/wear/' . $product['p_featured_photo'])) : ?>
                                                                <img src="./admin/uploads/wear/<?= ($product['p_featured_photo']); ?>" alt="<?= ($product['p_name']); ?>" title="<?= ($product['p_name']); ?>" class="food-img" style="width:100%; height:auto; object-fit: cover;">
                                                            <?php else : ?>
                                                                <img src="assets/jaipur-new-logo.png" alt="<?= ($product['p_name']); ?>" title="<?= ($product['p_name']); ?>" class="food-img" style="width:100%; height:auto; object-fit: cover;">
                                                            <?php endif; ?>
                                                        </a>
                                                ...
                                                <div class="product-description">
                                                    <div class="caption">
                                                        <div class="title-rating clearfix">
                                                            <h4 class="product-title"><a class="food-title2" href="wear-details.php?p_id=<?= $product['p_id']; ?>"><?= $product['p_name']; ?></a></h4>
                                                        </div>
                                                        <div class="price-cartbtn clearfix">
                                                            <p class="food-price">Rs.<?= $product['p_current_price']; ?></p>
                        ...
                        <?php }
                            ...
                            echo "<p>No products available in this endcategory.</p>";
                        } ?>
                    </div>
