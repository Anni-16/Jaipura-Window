...
if (isset($_GET['sub_cat_id'])) {
    ...
    $statement = $pdo->prepare("SELECT * FROM tbl_decorate_sub_category WHERE sub_cat_id = ? AND show_status = 1");
    $statement->execute([$url]);
    $row = $statement->fetch(PDO::FETCH_ASSOC);

    if (!$row) {
        // Redirect if subcategory not found
        header('location: decorate-category.php');
        exit;
    }
...
<head>
    ...
    <title>Welcome To Jaipur Window - Decorate Categories Filtaration Products | Jaipur Window</title>
        ...
        <div class="breadcrumb-main">
            <div class="container">
                <div class="breadcrumb-container">
                    <h2 class="page-title"><?= $sub_cat_name; ?></h2>
                    <ul class="breadcrumb">
            ...
            <div class="row">
                ...
                <div id="content" class="col-sm-9  all-blog my-baby-pink" style="  padding: 20px 40px;">
                    <div class="category-refine">
                        <h3><strong>Refine Search By Decorate Sub Categories</strong></h3>
                        <div class="row">
                            <div class="col-sm-12">
                                <ul>
                                    <?php
                                    $i = 0;
                                    $statement = $pdo->prepare("SELECT * FROM tbl_decorate_sub_category WHERE show_status = 1 ORDER BY sub_cat_name ASC");
                                    $statement->execute();
                                    $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($result as $row) {
                                        $i++;
                                    ?>
                                        <li>
                                            <a href="decorate-filter.php?sub_cat_id=<?= $row['sub_cat_id']; ?>"><?= $row['sub_cat_name']; ?></a>
                                        </li>
                                    <?php } ?>
                                </ul>
                            </div>
                                        ...
                                        <div class="food-cart-box" data-product-id="1">
                                            <div class="food-box">
                                                <div class="product-thumb">
                                                    <div class="image">
                                                        <a href="decorate-details.php?d_id=<?= $product['d_id']; ?>" class="thumb-image">
                                                            <?php if (!empty($product['d_photo']) && file_exists('./admin/uploads/decorate/' . $product['d_photo'])) : ?>
                                                                <img src="./admin/uploads/decorate/<?= ($product['d_photo']); ?>" alt="<?= ($product['d_name']); ?>" title="<?= ($product['d_name']); ?>" class="food-img" style="width:100%; height:auto; object-fit: cover;">
                                                            <?php else : ?>
                                                                <img src="assets/jaipur-new-logo.png" alt="<?= ($product['d_name']); ?>" title="<?= ($product['d_name']); ?>" class="food-img" style="width:100%; height:auto; object-fit: cover;">
                                                            <?php endif; ?>
                                                        </a>
                                                ...
                                                <div class="product-description">
                                                    <div class="caption">
                                                        <div class="title-rating clearfix">
                                                            <h4 class="product-title"><a class="food-title2" href="decorate-details.php?d_id=<?= $product['d_id']; ?>"><?= $product['d_name']; ?></a></h4>
                                                        </div>
                                                        <div class="price-cartbtn clearfix">
                                                            <p class="food-price">
                                                                Rs.<?= $product['d_current_price']; ?>
                                                            </p>
                                                        </div>
