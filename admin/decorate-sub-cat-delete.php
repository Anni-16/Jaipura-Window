...
if(!isset($_REQUEST['id'])) {
	...
	if( $total == 0 ) {
        ...
        $image = $result['sub_cat_image'];

        // Delete the image file
        if ($image != '' && file_exists('./uploads/decorate/subcategory/' . $image)) {
            unlink('./uploads/decorate/subcategory/' . $image);
        }

        // Delete the category record
        $statement = $pdo->prepare("DELETE FROM tbl_decorate_sub_category WHERE sub_cat_id=?");
        $statement->execute(array($_REQUEST['id']));
    }
}
?>
